...
: "${DATASET:?Set DATASET to the preprocessed ARC-AGI-2 dataset directory}"
: "${CHECKPOINT:?Set CHECKPOINT to the ARC-AGI-2 PTRM checkpoint}"
[[ -d "${DATASET}" ]] || { echo "Missing dataset: ${DATASET}" >&2; exit 2; }
[[ -f "${CHECKPOINT}" ]] || { echo "Missing checkpoint: ${CHECKPOINT}" >&2; exit 2; }

OUTPUT_DIR="${OUTPUT_DIR:-${RRM_PROJECT_ROOT}/artifacts/agi-2/ptrm}"
CANDIDATE_COUNT="${CANDIDATE_COUNT:-25}"
DEPTH="${DEPTH:-16}"
GLOBAL_BATCH_SIZE="${GLOBAL_BATCH_SIZE:-32}"
LATENT_NOISE_SCALE="${LATENT_NOISE_SCALE:-0.2}"
SEED="${SEED:-0}"
...
COMMAND=(
  ...
  --task arc-agi-2 --checkpoint "${CHECKPOINT}"
  ...
  --device "${RRM_DEVICE}" --latent-noise-scale "${LATENT_NOISE_SCALE}"
  "${EXTRA_ARGS[@]}"
)
