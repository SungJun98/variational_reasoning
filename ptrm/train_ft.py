...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fine-tune a PTRM/TRM checkpoint with IVON, SOAP, or EVON.")
...
def prepare_run(args: argparse.Namespace, schedule: dict[str, Any]) -> tuple[Path, Path]:
    ...
    status_path.write_text(f"# PTRM {args.optimizer.upper()} Fine-Tuning\n\n")
...
def main() -> None:
    ...
    wandb_run = utils.init_wandb(
        args, run_root, schedule, job_type=f"ptrm_{args.optimizer}_ft"
    )
